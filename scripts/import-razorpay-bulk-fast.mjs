/**
 * Fast full-replace import of a Razorpay dump into RazorpayPayment +
 * ConsolidatedPayment. Uses chunked multi-row INSERTs (no per-row scans).
 *
 * RazorpayPayment has no unique key on the transaction id, so this does a clean
 * TRUNCATE + fresh insert (the CSV is the full source of truth for the dump).
 * ConsolidatedPayment Razorpay rows are replaced (DELETE sourceName + insert).
 *
 * Usage: node scripts/import-razorpay-bulk-fast.mjs "<csv path>"
 */
import fs from 'fs';
...
const CHUNK = 1000;
const TABLE = 'RazorpayPayment';
const LABEL = 'Razorpay';
const TX_COL = 'settlement_utr_/_transaction_id';

const csvPath = process.argv[2];
if (!csvPath) {
  console.error('Usage: node scripts/import-razorpay-bulk-fast.mjs "<csv path>"');
  process.exit(1);
}
...
async function main() {
  ...
  // ---- RazorpayPayment: clean full replace ----
  ...
  // ---- ConsolidatedPayment: replace Razorpay rows ----
  ...
  const [{ c: rzp }] = await prisma.$queryRawUnsafe(`SELECT COUNT(*) AS c FROM \`${TABLE}\``);
  ...
  console.log(`\nDone. ${TABLE}=${Number(rzp)} rows, ConsolidatedPayment(${LABEL})=${Number(con)} (mapped ${Number(withEnr)})`);
}
