/**
 * Fast full-replace import of an Early dump into EarlyPayment + ConsolidatedPayment.
 * Usage: node scripts/import-early-bulk-fast.mjs "<csv path>"
 */
import fs from 'fs';
...
const CHUNK = 1000;
const TABLE = 'EarlyPayment';
const LABEL = 'Early';
const TX_COL = 'settlement_utr_/_transaction_id';

const csvPath = process.argv[2];
if (!csvPath) {
  console.error('Usage: node scripts/import-early-bulk-fast.mjs "<csv path>"');
  process.exit(1);
}
...
async function main() {
  ...
  // Prefer DELETE over TRUNCATE to avoid lock issues seen on RazorpayPayment
  ...
  const [{ c: early }] = await prisma.$queryRawUnsafe(`SELECT COUNT(*) AS c FROM \`${TABLE}\``);
  ...
  console.log(`\nDone. ${TABLE}=${Number(early)} rows, ConsolidatedPayment(${LABEL})=${Number(con)} (mapped ${Number(withEnr)})`);
}
