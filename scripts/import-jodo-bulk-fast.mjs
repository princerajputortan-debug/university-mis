/**
 * Fast full-replace import of a Jodo dump into JodoPayment + ConsolidatedPayment.
 * Usage: node scripts/import-jodo-bulk-fast.mjs "<csv path>"
 */
import fs from 'fs';
...
const CHUNK = 1000;
const TABLE = 'JodoPayment';
const LABEL = 'Jodo';
const TX_COL = 'settlement_utr_/_transaction_id';

const csvPath = process.argv[2];
if (!csvPath) {
  console.error('Usage: node scripts/import-jodo-bulk-fast.mjs "<csv path>"');
  process.exit(1);
}
...
async function getAmountColumn() {
  ...
  return { amountCol: col.Field, allCols: new Set(cols.map((c) => c.Field)) };
}

async function main() {
  const { amountCol, allCols } = await getAmountColumn();
  const hasEnrollmentId = allCols.has('enrollmentId');
  const hasBatchId = allCols.has('batchId');
  ...
  for (let i = 0; i < records.length; i += CHUNK) {
    ...
    const insertCols = ['id', 'date', TX_COL, 'enrollment_id', amountCol, 'mode', 'discounted_course_fee', '1st_emi', 'tenure'];
    if (hasBatchId) insertCols.push('batchId');
    if (hasEnrollmentId) insertCols.push('enrollmentId');

    const values = chunk.map((r) => {
      id += 1;
      // TX column on JodoPayment is bigint — insert numeric when possible
      const txSql = /^\d+$/.test(r.transactionId) ? r.transactionId : sqlStr(r.transactionId);
      const parts = [
        id,
        sqlStr(r.date),
        txSql,
        sqlStr(r.enrollmentText),
        sqlNum(r.amount),
        sqlStr(r.mode),
        sqlNum(r.discountedCourseFee),
        sqlNum(r.firstEmi),
        sqlNum(r.tenure),
      ];
      if (hasBatchId) parts.push(sqlNum(r.batchId));
      if (hasEnrollmentId) parts.push(sqlNum(r.enrollmentId));
      return `(${parts.join(', ')})`;
    }).join(',\n');

    const colList = insertCols.map((c) => `\`${c}\``).join(', ');
    await prisma.$executeRawUnsafe(`INSERT INTO \`${TABLE}\` (${colList}) VALUES ${values}`);
  ...
  const [{ c: jodo }] = await prisma.$queryRawUnsafe(`SELECT COUNT(*) AS c FROM \`${TABLE}\``);
  ...
  console.log(`\nDone. ${TABLE}=${Number(jodo)} rows, ConsolidatedPayment(${LABEL})=${Number(con)} (mapped ${Number(withEnr)})`);
}
