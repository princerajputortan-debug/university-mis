/**
 * Fast full-replace import of a payment dump into a legacy payment table +
 * ConsolidatedPayment. Uses chunked multi-row INSERTs (no per-row scans).
 *
 * The legacy tables (RazorpayPayment/EarlyPayment/JodoPayment/PropelldPayment)
 * share the same columns and have no unique key on the transaction id, so this
 * does a clean TRUNCATE + fresh insert (the CSV is the full source of truth).
 * ConsolidatedPayment rows for the source are replaced (DELETE + insert).
 *
 * Usage: node scripts/import-payment-bulk-fast.mjs <razorpay|early|jodo|propelld> "<csv path>"
 */
import fs from 'fs';
...
const SOURCE_MAP = {
  razorpay: { table: 'RazorpayPayment', label: 'Razorpay' },
  early: { table: 'EarlyPayment', label: 'Early' },
  jodo: { table: 'JodoPayment', label: 'Jodo' },
  propelld: { table: 'PropelldPayment', label: 'Propelld' },
};

const type = process.argv[2];
const csvPath = process.argv[3];
if (!type || !csvPath || !SOURCE_MAP[type]) {
  console.error('Usage: node scripts/import-payment-bulk-fast.mjs <razorpay|early|jodo|propelld> "<csv path>"');
  process.exit(1);
}
const { table: TABLE, label: LABEL } = SOURCE_MAP[type];
...
async function main() {
  ...
  console.log(`[${LABEL}] Parsed ${parsed.data.length} rows → ${records.length} unique transactions (enrollment mapped: ${mapped})`);

  // ---- legacy table: clean full replace ----
  ...
  // ---- ConsolidatedPayment: replace this source's rows ----
  ...
  const [{ c: rows }] = await prisma.$queryRawUnsafe(`SELECT COUNT(*) AS c FROM \`${TABLE}\``);
  ...
  const [{ s: sum }] = await prisma.$queryRawUnsafe(`SELECT COALESCE(SUM(amount),0) AS s FROM ConsolidatedPayment WHERE sourceName = ${sqlStr(LABEL)}`);
  console.log(`\nDone. ${TABLE}=${Number(rows)} rows, ConsolidatedPayment(${LABEL})=${Number(con)} (mapped ${Number(withEnr)}, sum ${Number(sum)})`);
}
